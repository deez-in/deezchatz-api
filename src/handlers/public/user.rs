...
use crate::{
    auth::google_oauth::google_oauth_pkce,
    auth::signature::AuthenticatedUser,
    db::user::{delete_user_account, put_user_report, resolve_user_id_by_email},
    error::AppError,
    models::api::auth::OAuthDeleteReq,
    models::api::user::{DeleteAccountResp, ReportUserReq, ReportUserResp},
    state::AppState,
};

pub async fn delete_account_vxeddsa(
...
pub async fn delete_account_by_oauth(
    ...
    let user_id = match resolve_user_id_by_email(&state, &identity.email).await? {
        Some(id) => id,
        None => {
    ...
    tracing::info!(
        ...
        "User account deleted successfully via OAuth"
    );
