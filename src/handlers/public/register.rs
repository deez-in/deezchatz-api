...
use crate::{
    auth::google_oauth::{google_oauth_id_token, google_oauth_pkce},
    ...
    models::{
        api::{
            auth::{GoogleIdTokenReq, GoogleIdTokenResp, OAuthRegisterReq},
            device::{RegisterDeviceReq, RegisterDeviceResp},
        },
        db::temp_registration::TempRegistration,
    },
    state::AppState,
};
...
pub async fn register_google_pkce(
    ...
    Json(req): Json<OAuthRegisterReq>,
) -> Result<Json<GoogleIdTokenResp>, AppError> {
    let identity = google_oauth_pkce(
        &state,
        &req.code,
    ...
    let resp = initiate_registration(&state, &identity, &req.i_key).await?;

    tracing::info!(
        user_id = %resp.user_id,
        email = %resp.email,
        "Google OAuth PKCE registration initiated successfully"
    );

    Ok(Json(resp))
}

pub async fn register_google_id_token(
    State(state): State<AppState>,
    Json(req): Json<GoogleIdTokenReq>,
) -> Result<Json<GoogleIdTokenResp>, AppError> {
    let identity = google_oauth_id_token(&state, &req.id_token).await?;
    ...
    tracing::info!(
        user_id = %resp.user_id,
        email = %resp.email,
        "Google OAuth ID token registration initiated successfully"
    );
